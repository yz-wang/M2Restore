...
def calc_ssim(im1, im2):
    ...
    im1_y = cv2.cvtColor(im1, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
    im2_y = cv2.cvtColor(im2, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
    ans = [compare_ssim(im1_y, im2_y, data_range=1, channel_axis=-1)]
    return ans
...
def validation(net, val_data_loader, device, exp_name, save_tag=False):
    ...
    with torch.no_grad():
        for (input_im, gt, imgid) in tqdm(val_data_loader):
            input_im, gt = input_im.cuda(), gt.cuda()
            ...
            if save_tag:
                # print()
                save_image(pred_image, imgid, exp_name, 'REAL')
...
def save_image(pred_image, image_name, exp_name, category):
    ...
    for ind in range(batch_num):
        ...
        print(image_name_1)
        utils.save_image(pred_image_images[ind], './AdaIR_results/{}/{}/{}'.format(category,exp_name,image_name_1))
